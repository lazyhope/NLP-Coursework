...
def train_tfidf_logistic_steps():
    X_train, y_train, X_val, y_val, X_test, y_test = load_data()
    
    # Create TfidfVectorizer and transform text data into TF-IDF weighted BoW representation
    ...
    n_epochs = 10
    # Training loop: iterate over epochs, recording metrics every 100 steps.
    ...
    plt.title("Learning Curve (F1 Score) - TF-IDF + Logistic Regression", fontsize=18)
    plt.ylim(0,1)
    plt.grid(True)
    plt.legend(fontsize=16)
    plt.savefig("figures/TFIDF_logistic_learning_curve.png")
    plt.show()
    ...
    # --- Print misclassified examples from the test set ---
    # print("\n=== Misclassified Test Instances (class 1) ===")
    ...
    topn = 10
    top_pos_indices = np.argsort(coef)[-topn:]
    top_neg_indices = np.argsort(coef)[:topn]
